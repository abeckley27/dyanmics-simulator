...
def collision(b1, b2, iteration):
    if check_intersect(b1, b2) and (b1 not in ignore) and (b2 not in ignore):
        ...
        new.dx = (b1.area()*b1.x + b2.area()*b2.x) / (b1.area() + b2.area())
        new.dy = (b1.area()*b1.y + b2.area()*b2.y) / (b1.area() + b2.area())
...
if __name__ == "__main__":
    pretty_colors = raw_input('Enter y to run graphical simulation: ')
    print '==> ' + pretty_colors.lower()

    source = raw_input('Enter y to generate random ball positions: ')
    print '==> ' + source.lower()
